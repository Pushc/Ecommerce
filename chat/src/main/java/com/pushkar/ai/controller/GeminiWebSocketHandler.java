package com.pushkar.ai.controller;


import com.pushkar.ai.entity.Product;
import com.pushkar.ai.repo.ProductRepository;
...
public class GeminiWebSocketHandler extends TextWebSocketHandler {
    ...
    private boolean isEcommerceQuery(String userInput) {
        String[] productCategories = {"ac", "refrigerator", "fridge", "washing machine", "tv", "television","electronics","clothing","gadgets"};

        for (String category : productCategories) {
            if (userInput.contains(category) && (userInput.contains("under") || userInput.contains("below") || userInput.contains("above") || userInput.contains("greater than"))) {
                return true;
            }
        }
        return false;
    ...
    private String handleEcommerceQuery(String userInput) {
        try {
            ...
            if (isLessThan) {
                products = productRepository.findByCategoryAndPriceLessThan(detectedCategory, maxPrice);
            } else {
                products = productRepository.findByCategoryAndPriceGreaterThan(detectedCategory, maxPrice);
            }
            ...
            StringBuilder response = new StringBuilder("Here are some " + detectedCategory + "s " + (isLessThan ? "under" : "above") + " ₹" + maxPrice + ":");
