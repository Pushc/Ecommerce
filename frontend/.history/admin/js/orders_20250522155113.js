  ...
  orders.forEach(order => {
    ...
    row.innerHTML = `
      ...
      <td>
        ...
        ${order.status !== 'SUCCESS' ? `<button onclick="updateOrderStatus('${order.payment_id}', '${order.order_id}', 'SUCCESS')">✅ Mark as Paid</button>` : ""}
      </td>
    `;
...
function updateOrderStatus(paymentId, orderId, newStatus) {
  const items = prompt("Enter item names (optional, comma separated):", "");
  
  const url = `${apiUrl}/updateOrder?payment_id=${paymentId}&order_id=${orderId}&status=${newStatus}&items=${encodeURIComponent(items || "")}`;
